...
class Clipboard {
  ...
  typealias OnNewCopyHook = (_ item: HistoryItem, _ copiedFromMaccy: Bool) -> Void
  ...
  func onNewCopy(_ hook: @escaping (HistoryItem) -> Void) {
    onNewCopyHooks.append { item, _ in hook(item) }
  }
  ...
  func checkForChangesInPasteboard() { // swiftlint:disable:this cyclomatic_complexity
    ...
    let copiedFromMaccy = pasteboard.pasteboardItems?.contains(where: { $0.types.contains(.fromMaccy) }) == true
    ...
    if !copiedFromMaccy {
      ...
      // Selecting an item from Maccy writes it back to the system clipboard with
      // a Maccy marker. Do not record that internal clipboard write as a new
      // history event; only external copies should enter history.
      return
    }
    ...
    onNewCopyHooks.forEach({ $0(historyItem, copiedFromMaccy) })
  }
